...
function extractAllIngredients(description) {
  ...
  const ingredientPatterns = [
    ...
    // 「原材料」パターン（単独）
    /原材料[】：:\s]+(.*?)(?=\s+内容量|\s+賞味期限|\s+保存方法|\s+区分|\s+お召し上がり方|\s+摂取|\s+使用上|\s+【|$)/is,
    // 「全成分」パターン
...
if (extractedData.length > 0) {
  ...
  fs.writeFileSync('extracted-all-ingredients-v3.ndjson', ndjson);

  console.log(`✅ extracted-all-ingredients-v3.ndjson を作成しました（${extractedData.length}件）\n`);
  console.log('次のステップ:');
  console.log('  1. merge-all-ingredients.mjs を実行してマージ');
